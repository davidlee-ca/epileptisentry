...
common_key = f"{subject_id}_{replay_file_number}"
source_key = f"{subject_id}/{common_key}.edf"
export_key = f"{subject_id}/{common_key}.csv"
temporary_source = f"/tmp/{common_key}.edf"
temporary_export = f"/tmp/{common_key}.csv"

s3 = boto3.client('s3')
s3.Bucket(source_bucket).download_file(source_key, temporary_source)

f = pyedflib.EdfReader(temporary_filename)
...
if tape_type == 2:
    np.delete(exportArray, -1, axis=1)
if tape_type == 3:
    np.delete(exportArray, [5, 10, 13, 18, 23], axis=1)
